...
def main(cfg: OmegaConf) -> None:
    ...
    if cfg.util_args.eval_mode == True:
        ...
        loaded_model = torch.load(cfg.model_load, weights_only=False)['state_dict']
        ...
        resume_checkpoint = cfg.save_model_dir + '/last.ckpt' if os.path.exists(
            cfg.save_model_dir + '/last.ckpt') else None
        trainer = Trainer(
            callbacks=ModelCheckpoint(dirpath=cfg.save_model_dir, **cfg.trainer.checkpoint_kwargs),
            logger=metrics_logger,
