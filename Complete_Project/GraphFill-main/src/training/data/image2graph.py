...
class Image2Graph(object):
    ...
    def get_data(self, img, mask):
        ...
        for idx, nseg in enumerate(self.segments):
            if self.segments_mask is None or mask.max() == 0:
                seg = slic(masked_img, n_segments=nseg, start_label=0, compactness=10.0, max_num_iter=10, sigma=1,
                           spacing=None, convert2lab=None, enforce_connectivity=True, mask=None)
                seg = torch.from_numpy(seg)
            else:
                # masked_img or img ?
                segkn = slic(masked_img, n_segments=nseg, compactness=10.0, max_num_iter=10, sigma=1, spacing=None,
                             convert2lab=None, enforce_connectivity=True, mask=1 - mask[0])
                start_label = segkn.max() + 1
                nsegm = self.segments_mask[idx]
                segun = slic(masked_img, n_segments=nsegm, compactness=10.0, max_num_iter=10, sigma=1, spacing=None,
                             convert2lab=None, enforce_connectivity=True, mask=mask[0])
    ...
    def verify_data(self, data, seg_dict, img, masked_img, maskimo):
        for key in seg_dict.keys():
            ...
            imwb = mark_boundaries(inpim.numpy(), maskim.numpy()[:, :, 0], color=(1, 0, 0), outline_color=None,
            ...
            plt.figure()
            imwb = mark_boundaries(gtim.numpy(), maskim.numpy()[:, :, 0], color=(1, 0, 0), outline_color=None,
            ...
            plt.show(block=False)
            if key == 100:
                imwb = mark_boundaries(inpim.numpy(), segments * maskim.numpy()[:, :, 0], color=(1, 1, 0),
                                       outline_color=None, mode='outer', background_label=0)
                imwb = mark_boundaries(imwb, segments * (1 - maskim.numpy()[:, :, 0]), color=(1, 0, 0),
                                       outline_color=None, mode='outer', background_label=0)
                plt.figure()
                plt.imshow(imwb)
                plt.axis('off')
    ...
    def verify_data_bkp(self, data, seg_dict):
        for key in seg_dict.keys():
            ...
            segments_ids = np.unique(segments)
            centers = np.array([np.mean(np.nonzero(segments == i), axis=1) for i in segments_ids])
            imwb = mark_boundaries(inpim.numpy(), segments, color=(1, 1, 0), outline_color=None, mode='outer',
                                   background_label=0)
            plt.imshow(imwb)
            plt.scatter(centers[eds][:, :, 1], centers[eds][:, :, 0], c='y')
            ...
            # edsp=SparseTensor(row=eds[0], col=eds[1], sparse_sizes=(masked.shape[0], masked.shape[0])).t()
            plt.figure()
            imwb = mark_boundaries(gtim.numpy(), segments, color=(1, 1, 0), outline_color=None, mode='outer',
                                   background_label=0)
            plt.imshow(imwb)
            plt.scatter(centers[eds][:, :, 1], centers[eds][:, :, 0], c='y')
            for idx, (p1, p2) in enumerate(centers): plt.text(p2, p1, str(idx), c='red')
